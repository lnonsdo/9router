 ...
 */
export async function handleStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, userAgent, body, stream, translatedBody, finalBody, requestStartTime, connectionId, sessionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, customToolNames, streamController, onStreamComplete, streamDetailId, pxpipe, reqTag, log }) {
  if (onRequestSuccess) {
    Promise.resolve()
  ...
  saveRequestDetail(buildRequestDetail({
    provider, model, connectionId, sessionId,
 ...
 */
export function buildOnStreamComplete({ provider, model, connectionId, sessionId, apiKey, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest, pxpipe, reqTag, log }) {
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    saveUsageStats({ provider, model, tokens: usage, connectionId, sessionId, apiKey, endpoint: clientRawRequest?.endpoint, label: "STREAM USAGE", silent: true });
