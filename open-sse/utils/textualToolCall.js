export function stripZeroWidth(value) {
  ...
  if (Array.isArray(value)) return value.map(item => stripZeroWidth(item));
...
export function isValidToolCallHeaderPrefix(candidate) {
  ...
  if (textAfterWhitespace.length === 0) return true;
  if (!leadingWhitespace.includes("\n")) return false;
  const expectedText = "Arguments:";
  if (expectedText.startsWith(textAfterWhitespace)) return true;
  if (textAfterWhitespace.startsWith(expectedText)) return true;
  return false;
}

export function parseTextualToolCallCandidate(text) {
  if (typeof text !== "string") return null;
  const normalized = text.replace(/[\u200B-\u200D\uFEFF]/g, "");
  const toolCallIndex = normalized.lastIndexOf("[Tool call:");
  if (toolCallIndex < 0) {
    const lastParen = normalized.lastIndexOf("(");
    if (lastParen !== -1 && "(empty)[Tool call:".startsWith(normalized.slice(lastParen))) return { kind: "partial" };
    const lastBracket = normalized.lastIndexOf("[");
    if (lastBracket !== -1 && "[Tool call:".startsWith(normalized.slice(lastBracket))) return { kind: "partial" };
    return null;
  }
  const candidate = normalized.slice(toolCallIndex);
  if (!isValidToolCallHeaderPrefix(candidate)) return null;
...
export function containsTextualToolCallMarker(text) {
